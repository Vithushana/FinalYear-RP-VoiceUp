...
def officer_view(complaint_id: int, db: Session = Depends(get_db)):
    ...
    # 1) Supporting ML prediction
    ml_result = predict_from_text(complaint.text_expanded or "")
    ...
    # 3) Read cached OSM values from DB
    poi_list = []
    ...
    # 4) Build officer output using ML text severity + GIS + recurring
    officer_output = build_officer_output(
        complaint_id=complaint.id,
        category=complaint.category,
        expanded_text=complaint.text_expanded or "",

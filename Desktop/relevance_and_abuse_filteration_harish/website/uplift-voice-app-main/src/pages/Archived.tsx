...
const mockIssues = [
  {
    _id: "1",
    title: "Archived Road Crack",
    description: "A road crack near the town center has been archived for future reference.",
  ...
  {
    _id: "2",
    title: "Archived Pothole",
    description: "Potholes on the main road have been archived for future reference.",
...
const Archived: React.FC = () => {
  ...
  const fetchIssues = async () => {
    try {
      setLoading(true);
      const data = await ApiService.getIssues(50, 0, 'Archived');
      setIssues(mockIssues.map((issue) => ({ ...issue, status: "Archived" })));
    } catch (error) {
        ...
        <div className="p-6">
              ...
              return (
                <Card key={issue._id} className={`bg-white border-2 ${getPriorityBorderColor(issue.priority)} hover:shadow-lg transition-shadow`}>
                  <CardContent className="p-4">
                    ...
                    <div className="mb-3">
                      <img
                        src={issue.userUploadImages?.[0] || archivedImages[index % archivedImages.length]}
                        alt={issue.title}
                    ...
                    <div className="flex items-center justify-end">
                      <Button
                        ...
                        className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-1 text-sm rounded-md"
                      >
                        details
                      </Button>
                    </div>
          ...
          {issues.length === 0 && (
            <div className="text-center py-12">
              <div className="text-gray-500 text-lg">No archived issues found</div>
            </div>
          )}
        </div>
      </div>

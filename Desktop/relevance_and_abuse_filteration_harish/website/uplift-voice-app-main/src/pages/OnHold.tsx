...
const OnHold: React.FC = () => {
  ...
  const fetchIssues = async () => {
    try {
      ...
      if (officerId) {
        const data = await ApiService.getOfficerIssues(officerId, 'hold', 50);
        const formattedIssues = (data.posts || []).map((issue: any) => ({
          ...issue,
          userUploadImages: (issue.userUploadImages || issue.images || []).map((img: string) => formatImageUrl(img))
        }));
        setIssues(formattedIssues);
      }
    } catch (error) {
      console.error('Error fetching on hold issues:', error);
      setIssues([]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1">
        <div className="bg-white border-b p-4 flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-800">On Hold</h1>
        ...
        <div className="p-6">
            ...
            {issues.map((issue) => (
              <Card key={issue.id || issue._id} className={`bg-white border-2 ${getPriorityBorderColor(issue.priority)} hover:shadow-lg transition-shadow h-full`}>
                <CardContent className="p-4 h-full flex flex-col">
                  ...
                  <div className="mb-3">
                    <img
                      src={issue.userUploadImages?.[0] || '/images/image 15 (1).png'}
                      alt={issue.title}
                  ...
                  <div className="flex items-center justify-end">
                    <Button
                      ...
                      className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-1 text-sm rounded-md"
                    >
                      details
                    </Button>
                  </div>
          ...
          {issues.length === 0 && (
            <div className="text-center py-12">
              <div className="text-gray-500 text-lg">No issues on hold found</div>
            </div>
          )}
        </div>
      </div>

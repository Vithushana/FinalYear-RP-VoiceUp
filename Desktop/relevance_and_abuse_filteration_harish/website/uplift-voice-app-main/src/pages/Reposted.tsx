...
const Reposted: React.FC = () => {
    ...
    const fetchIssues = async () => {
        try {
            setLoading(true);
            const data = await ApiService.getRepostedIssues(50, 0);
            ...
            console.error('Error fetching reposted issues:', error);
            setIssues([]);
        } finally {
            setLoading(false);
        }
        ...
        <div className="min-h-screen bg-gray-50 flex">
            ...
            <div className="flex-1">
                <div className="bg-white border-b p-4 flex items-center justify-between">
                    <h1 className="text-2xl font-bold text-gray-800">Reposted Issues</h1>
                ...
                <div className="p-6">
                    <div className="mb-4 text-sm text-gray-600">
                        <p>Issues where citizens have replied to your reports or retagged their posts</p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {issues.map((issue: any) => (
                            <Card key={issue.id} className={`bg-white border-2 ${getPriorityBorderColor(issue.priority || '2')} hover:shadow-lg transition-shadow h-full`}>
                                <CardContent className="p-4 h-full flex flex-col">
                                    ...
                                    <div className="mb-4 flex-grow">
                                        ...
                                        <div className="text-xs text-gray-500 space-y-1">
                                            <div><span className="font-medium">Type:</span> {issue.type || issue.issue_type}</div>
                                            <div><span className="font-medium">Location:</span> {issue.location}</div>
                                            <div><span className="font-medium">Posted:</span> {new Date(issue.created_at).toLocaleDateString()}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mb-3 mt-auto">
                                        <StatusProgression
                                            ...
                                            issueId={issue.id}
                                            onStatusUpdate={(newStatus) => handleStatusUpdate(issue.id, newStatus)}
                                        />
                                    </div>
                    ...
                    {issues.length === 0 && (
                        <div className="text-center py-12">
                            <div className="text-gray-500 text-lg">No reposted issues found</div>
                            <p className="text-sm text-gray-400 mt-2">
                                Reposted issues appear when citizens reply to your reports or retag their posts
                            </p>
                        </div>
                    )}
                </div>

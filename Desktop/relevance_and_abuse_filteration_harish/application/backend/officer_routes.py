...
def get_stats():
    ...
    try:
        ...
        # Total issues
        total_issues = Post.query.count()
        
        # Issues by status
        submitted = Post.query.filter_by(status='submitted').count()
        seen = Post.query.filter_by(status='seen').count()
        verified = Post.query.filter_by(status='verified').count()
        in_progress = Post.query.filter_by(status='in_progress').count()
        completed = Post.query.filter_by(status='completed').count()
        closed = Post.query.filter_by(status='closed').count()  # ✅ ADDED CLOSED COUNT
        on_hold = Post.query.filter_by(status='hold').count()
        
        # Achievements count (posts that are closed with achievement records)
        achievements_count = Achievement.query.count()  # ✅ FIXED: Now counts actual achievements
        
        # Recent activity (last 7 days)
        week_ago = datetime.utcnow() - timedelta(days=7)
        recent_issues = Post.query.filter(Post.created_at >= week_ago).count()
        
        # Issues by type
        issue_types = db.session.query(
            Post.issue_type,
            func.count(Post.id).label('count')
        ).group_by(Post.issue_type).all()
        ...
        stats = {
            ...
            'completed': completed,
            'closed': closed,  # ✅ ADDED CLOSED
            'hold': on_hold,
            'on_hold': on_hold,  # Alias
            'recent_activity': recent_issues,
            'achievements': achievements_count,  # ✅ FIXED: Now shows actual count

...
import { Bell, Search, User, CheckCircle } from 'lucide-react';
...
const Completed: React.FC = () => {
  const [issues, setIssues] = useState([
    {
      id: "1",
      title: "Completed Road Crack",
      description: "A road crack near the town center has been repaired successfully.",
    ...
    {
      id: "2",
      title: "Completed Pothole",
      description: "Potholes on the main road have been filled successfully.",
  ...
  const fetchIssues = async () => {
    try {
      setLoading(true);
      const data = await ApiService.getIssues(50, 0, 'Completed');
      setIssues(data);
    } catch (error) {
            ...
            {issues.map((issue, index) => {
              ...
              <Card key={issue._id} className={`bg-white border-2 ${getPriorityBorderColor(issue.priority)} hover:shadow-lg transition-shadow h-full`}>
                <CardContent className="p-4 h-full flex flex-col">
                  ...
                  <div className="mb-3">
                    <img
                      src={issue.userUploadImages?.[0] || completedImages[index % completedImages.length]}
                      alt={issue.title}

...
const OnHold: React.FC = () => {
  const [issues, setIssues] = useState<Issue[]>([]); // ✅ use Issue[]
  const [loading, setLoading] = useState(true);
  const [userName, setUserName] = useState<string>(""); // ✅ dynamic logged-in user
  ...
  const fetchIssues = async () => {
    try {
      setLoading(true);
      const data = await ApiService.getIssues(50, 0, "On Hold");
      setIssues(data);
    } catch (error) {

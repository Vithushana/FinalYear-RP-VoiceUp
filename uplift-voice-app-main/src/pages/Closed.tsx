...
const Closed: React.FC = () => {
  const [issues, setIssues] = useState([
    {
      id: "1",
      title: "Closed Road Crack",
      description: "A road crack near the town center has been closed after successful repair.",
    ...
    {
      id: "2",
      title: "Closed Pothole",
      description: "Potholes on the main road have been closed after successful repair.",
  ...
  const fetchIssues = async () => {
    try {
      setLoading(true);
      const data = await ApiService.getIssues(50, 0, 'Closed');
      setIssues(data);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1">
        <div className="bg-white border-b p-4 flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-800">Closed</h1>
        ...
        <div className="p-6">
            ...
            {issues.map((issue, index) => {
              ...
              <Card key={issue._id} className={`bg-white border-2 ${getPriorityBorderColor(issue.priority)} hover:shadow-lg transition-shadow h-full`}>
                <CardContent className="p-4 h-full flex flex-col">
                  ...
                  <div className="mb-3">
                    <img
                      src={issue.userUploadImages?.[0] || closedImages[index % closedImages.length]}
                      alt={issue.title}
                      className="w-full h-40 object-cover rounded-lg opacity-75"
                    />
                  </div>
          ...
          {issues.length === 0 && (
            <div className="text-center py-12">
              <div className="text-gray-500 text-lg">No closed issues found</div>
            </div>
          )}
        </div>
      </div>

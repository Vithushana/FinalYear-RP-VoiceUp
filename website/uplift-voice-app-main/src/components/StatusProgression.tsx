      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="flex items-start justify-center gap-2">
          {STATUS_STAGES.map((stage, index) => (
            <div key={stage.name} className="flex flex-col items-center gap-2 min-w-[60px] max-w-[60px]">
              <button
                onClick={() => handleStatusClick(stage.name.replace('\n', ' '), index)}
                className={`w-6 h-6 rounded-full transition-all duration-200 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-blue-400 ${index === currentIndex
              ...
              />
              <span className={`text-xs font-medium text-center leading-tight break-words ${index === currentIndex ? 'text-green-600' : 'text-gray-500'
                }`}>
                {stage.name.split('\n').map((line, lineIndex) => (
                  <div key={lineIndex} className="px-1">{line}</div>
                ))}
              </span>
            </div>
          ))}

...
const Inbox: React.FC = () => {
  ...
  const fetchIssues = async () => {
    try {
      ...
      if (officerId) {
        // Fetch officer's unseen issues (status='submitted')
        const data = await ApiService.getOfficerIssues(officerId, 'submitted', 50);
  ...
  const handleStatusUpdate = async (issueId: string, newStatus: string) => {
    try {
      // Get officer ID safely for cross-officer notifications
      const user = getUserData();
      const officerId = user?.id;
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="flex-1">
        <div className="bg-white border-b p-4 flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-800">Inbox - Unseen Issues</h1>
        ...
        <div className="p-6">
          ...
          {issues.length === 0 && (
            <div className="text-center py-12">
              <div className="text-gray-500 text-lg">No unseen issues found</div>
              <p className="text-sm text-gray-400 mt-2">New issues submitted by citizens will appear here</p>
            </div>
          )}
        </div>
      </div>
